...
def move_up(target: MapPoint):
    ...
    if up_point.x - next_platform.begin_x <= 6 and bot_status.player_moving and bot_status.player_direction == 'left':
        move_horizontal(MapPoint(up_point.x+3, p.y, 2))
        time.sleep(0.2)
    elif next_platform.end_x - up_point.x <= 6 and bot_status.player_moving and bot_status.player_direction == 'right':
        move_horizontal(MapPoint(up_point.x-3, p.y, 2))
        time.sleep(0.2)
...
def find_next_horizontal_point(start: MapPoint, target: MapPoint):
    ...
    if platform_start.end_x < platform_target.begin_x:
        if start.x in range(platform_target.begin_x + 3 - max_distance, platform_start.end_x + 1):
            ...
            return MapPoint(platform_start.end_x - 5, platform_start.y, 3)
    else:
        if start.x in range(platform_start.begin_x, platform_target.end_x - 3 + max_distance + 1):
            return target
        else:
            return MapPoint(platform_start.begin_x + 5, platform_start.y, 3)
