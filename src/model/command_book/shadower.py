...
#########################
    ...
    def step(target, tolerance):
        ...
        if abs(d_x) in Shadower.ShadowAssault.x_range and -d_y in Shadower.ShadowAssault.y_range:
            if Shadower.ShadowAssault.canUse():
                Shadower.ShadowAssault(target=target).execute()
                return
        if d_y in Shadower.ShadowAssault.y_range:
            if Shadower.ShadowAssault.canUse():
                Shadower.ShadowAssault(target=target).execute()
                return
        if abs(d_x) >= 26:
        ...
        if direction == "up":
            Shadower.move_up(next_p)
        elif direction == "down":
            Shadower.move_down(next_p)
        elif abs(d_x) >= 26:
            Shadower.hit_and_run(direction, next_p, tolerance)
        # elif abs(d_x) >= 20:
    ...
    def move_up(target):
        ...
        if dy <= 0:
            ...
            Shadower.JumpUp(target).execute()
        elif dy <= 41 and Shadower.ShadowAssault.canUse():
            Shadower.ShadowAssault(target=target).execute()
        else:
    ...
    def move_down(target):
        p = bot_status.player_pos
        dy = p[1] - target[1]
        if dy >= 24 and Shadower.ShadowAssault.canUse():
            Shadower.ShadowAssault(target=target).execute()
        else:
    ...
    class Buff(Command):
        ...
        def __init__(self):
            ...
            self.buffs: list[Skill] = [
                Shadower.MapleWarrior,
                Shadower.MapleWorldGoddessBlessing,
                Shadower.LastResort,
                Shadower.ForTheGuild,
                Shadower.HardHitter,
                Shadower.ShadowWalker,
                Shadower.PickPocket,
            ]
